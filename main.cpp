...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Matrix4x4 SWorldMatrix = MakeAffineMatrix(scales[0], rotate[0], translates[0]);
		Matrix4x4 EWorldMatrix = MatrixMultiply(MakeAffineMatrix(scales[1], rotate[1], translates[1]), SWorldMatrix);
		Matrix4x4 HWorldMatrix = MatrixMultiply(MakeAffineMatrix(scales[2], rotate[2], translates[2]), EWorldMatrix);

		Ssphere.center = { SWorldMatrix.m[3][0],SWorldMatrix.m[3][1],SWorldMatrix.m[3][2] };
		Esphere.center = { EWorldMatrix.m[3][0],EWorldMatrix.m[3][1],SWorldMatrix.m[3][2] };
		Hsphere.center = { HWorldMatrix.m[3][0],HWorldMatrix.m[3][1],HWorldMatrix.m[3][2] };

		Vector3 SsphereNdcVertex = Transform(Ssphere.center, worldViewProjectionMatrix);
		Vector3 SsphereScreenVertice = Transform(SsphereNdcVertex, viewportMatrix);

		Vector3 EsphereNdcVertex = Transform(Esphere.center, worldViewProjectionMatrix);
		Vector3 EsphereScreenVertice = Transform(EsphereNdcVertex, viewportMatrix);

		Vector3 HsphereNdcVertex = Transform(Hsphere.center, worldViewProjectionMatrix);
		Vector3 HsphereScreenVertice = Transform(HsphereNdcVertex, viewportMatrix);
		...
		ImGui::DragFloat3("rotates[0]", &rotate[0].x, 0.01f);
		ImGui::DragFloat3("scales[0]", &scales[0].x, 0.01f);
		ImGui::DragFloat3("translates[1]", &translates[1].x, 0.01f);
		ImGui::DragFloat3("rotates[1]", &rotate[1].x, 0.01f);
		ImGui::DragFloat3("scales[1]", &scales[1].x, 0.01f);
		ImGui::DragFloat3("translates[2]", &translates[2].x, 0.01f);
		ImGui::DragFloat3("rotates[2]", &rotate[2].x, 0.01f);
		...
		DrawSphere(Ssphere, worldViewProjectionMatrix, viewportMatrix, RED);
		DrawSphere(Esphere, worldViewProjectionMatrix, viewportMatrix, GREEN);
		DrawSphere(Hsphere, worldViewProjectionMatrix, viewportMatrix, BLUE);

		Novice::DrawLine((int)SsphereScreenVertice.x, (int)SsphereScreenVertice.y, (int)EsphereScreenVertice.x, (int)EsphereScreenVertice.y, WHITE);
		Novice::DrawLine((int)EsphereScreenVertice.x, (int)EsphereScreenVertice.y, (int)HsphereScreenVertice.x, (int)HsphereScreenVertice.y, WHITE);
